...
class MeetingMailer < Mailer
  ...
  def send_invalid_answer(doodle, rec, language)
    set_language_if_valid language
    sub = "FAILED:[#{doodle.project.name} - Umfrage #{doodle.id}]#{doodle.title}"
    recipients rec
    subject sub
    body :doodle => doodle
  ...
  def send_ak_answer(response, rec, language)
    ...
    sub = "SUCCESS:[#{doodle.project.name} - Umfrage #{doodle.id}]#{doodle.title}"
    recipients rec
    subject sub
  ...
  def receive_answer(answer)
    ...
    sub = "ANSWER:[#{doodle.project.name} - Umfrage #{doodle.id}]#{doodle.title}"
